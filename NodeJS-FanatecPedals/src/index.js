...
const argv = yargs(hideBin(process.argv))
  ...
  .option('BrakeDeadzoneIn', { alias: 'brake-deadzone-in', type: 'number' })
  .option('BrakeDeadzoneOut', { alias: 'brake-deadzone-out', type: 'number' })
  .option('ClutchDeadzoneIn', { alias: 'clutch-deadzone-in', type: 'number' })
  .option('ClutchDeadzoneOut', { alias: 'clutch-deadzone-out', type: 'number' })
...
const sharedJoyInfoEx = new JOYINFOEX();
let latestFrameAvailable = false;
let httpBatchId = 0;
...
function speak(text) {
  ...
  if (sayLib && os.platform() === 'win32') {
    try {
      sayLib.speak(text, undefined, undefined, (err) => {
        if (err) {
      ...
      console.error('TTS invocation failed, falling back to log:', err);
    }
  }
...
const frame = {
  ...
  brake_deadzone_in: brakeDeadzoneIn,
  brake_deadzone_out: brakeDeadzoneOut,
  clutch_deadzone_in: clutchDeadzoneIn,
  clutch_deadzone_out: clutchDeadzoneOut,
  ...
  brakeIdleMax: brakeIdleMax,
  brakeFullMin: brakeFullMin,
  clutchIdleMax: clutchIdleMax,
  clutchFullMin: clutchFullMin,
...
function refreshAxisDerived() {
  ...
  frame.clutch_deadzone_in = clutchDeadzoneIn;
  frame.clutch_deadzone_out = clutchDeadzoneOut;
  frame.brake_deadzone_in = brakeDeadzoneIn;
  frame.brake_deadzone_out = brakeDeadzoneOut;
  frame.clutchIdleMax = clutchIdleMax;
  frame.clutchFullMin = clutchFullMin;
  frame.brakeIdleMax = brakeIdleMax;
  frame.brakeFullMin = brakeFullMin;
}
...
const server = http.createServer((req, res) => {
  ...
  if (req.url === '/' || req.url === '') {
    ...
    if (req.method === 'GET') {
      const frames = latestFrameAvailable ? [frame] : [];
      const body = JSON.stringify({
        schemaVersion: 1,
        bridgeInfo: {
          batchId: (++httpBatchId) >>> 0,
          generatedAtUnixMs: Date.now(),
          framesInBatch: frames.length >>> 0
        },
        frames
      });
...
function pollLoop() {
  ...
  if (connectedThisLoop) {
    ...
    const gasLogicalPct = computeLogicalPct(gasValue, gasIdleMax, gasFullMin);
    const clutchLogicalPct = computeLogicalPct(clutchValue, clutchIdleMax, clutchFullMin);
    const brakeLogicalPct = computeLogicalPct(brakeValue, brakeIdleMax, brakeFullMin);
...
function broadcastFrame(nowUnixMs) {
  if (wss.clients.size > 0 && telemetryEnabledFlag) {
    const httpStart = performance.now();
    frame.receivedAtUnixMs = nowUnixMs;
...
function publishFrame(nowUnixMs) {
  broadcastFrame(nowUnixMs);
}

function computeLogicalPct(value, idleMax, fullMin) {
  if (value <= idleMax) return 0;
  if (value >= fullMin) return 100;
  if (fullMin <= idleMax) return 0;
  const delta = fullMin - idleMax;
  return Math.trunc((value - idleMax) * 100 / delta) >>> 0;
}
