...
It is easier to try than the newest WinUI app if you do not want to install the full modern desktop-app stack, but it is also slower due to scripting languages (Javascript and PowerShell).
...
- the **WinUI GUI** is the nicest and most complete, if you are willing to pay the dependency
...
However, I used those pedals for flight sims rudders, with a merged setup using Joystick Gremlin, and it works great... except when the clutch starts generating noise and my fighter moves in unexpected ways like when aligning my gun sight on an enemy target.  That meant a noisy clutch axis was no longer just "a little annoying." It could become a real in-sim problem:
...
```bash
fanatecmonitor.exe --vendor-id 0EB7 --product-id 1839 --flags 266 --iterations 0 --idle --affinitymask 268369920 --monitor-gas --gas-deadzone-out 100 --estimate-gas-deadzone-out --gas-min-usage 60 --sleep 100 --adjust-deadzone-out-with-minimum 89
```

Notes:

- `--estimate-gas-deadzone-out` learns from real usage over time instead of trusting a single pull
- `--adjust-deadzone-out-with-minimum 89` allows automatic downward correction, but never below 89
...
- `--ipc`
  Use experimental IPC "SPEAK" dispatch for speech instead of launching PowerShell. This requires an IPC listener. I wrote one here:
  https://github.com/rolex20/TelemetryVibShaker/blob/master/WebScripts/README.md
  Recommendation: don't use `--ipc` unless you know exactly why you need it.
