...
# -----------------------------------------------------------------------------
$Strings = @{
    Banner           = "Fanatec Pedals Monitor & Bridge v2.6.1 started."
...
# NEW: set this to the axis that moves when you press the brake pedal.
$BRAKE_AXIS  = 'Z'
...
try {
    ...
    $AxisMax = if (($JoyFlags -band 0x00000100) -ne 0) { 1023 } else { 65535 }
    ...
    # Derived thresholds and timing windows (legacy fields expected by PedDash)
    $State.axisMargin = [uint32]$AxisMargin
    $State.gasIdleMax = [uint32]$GasIdleMax
    $State.gasFullMin = [uint32]$GasFullMin
    $State.gas_timeout_ms = [uint32]($GasTimeout * 1000)
    $State.gas_window_ms = [uint32]($GasWindow * 1000)
    $State.gas_cooldown_ms = [uint32]($GasCooldown * 1000)

    # Estimator visibility fields (legacy)
    ...
    while ($Iterations -eq 0 -or $LoopCount -lt $Iterations) {
        ...
        # Brake: no deadzone/noise/drift logic yet; report linear physical->logical.
        $State.brake_logical_pct = [uint32](100 * $State.brakeValue / $AxisMax)
                        ...
                        if ($EstimateWindowPeakPercent -ge $GasMinUsage) {
                            if ($EstimateWindowPeakPercent -lt $BestEstimatePercent) {
                                ...
                                if (($State.currentTime - $LastEstimatePrintTime) -ge ($GasCooldown * 1000)) {
                                     ...
                                     $LastPrintedEstimate = [uint32]$BestEstimatePercent
                                }
        ...
        # --- Legacy/Debug state sync (MUST happen before Clone/Enqueue) ---
        ...
        $State.last_estimate_print_time = [uint32]$LastEstimatePrintTime
    $State.last_reconnect_time_ms = 0
        $State.currentPercent = [uint32]($State.gasValue * 100 / $AxisMax)
