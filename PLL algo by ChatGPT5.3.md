...
- Please implement improvements to PedDash (single self contained html dashboard app for racing sims)
- Carefully understand and analyze the source code for PedDash (provided below) in the next sections.
- Critical: First make the best plan to ensure you don't break anything and then proceed immediately to implement, all in one shot.
- PedDash has a lot of algorithms to try to obtain 1 frame in the next GET and i tried all the algorithms and none of them achieve the perfect 1 frame for more thn a couple of seconds.  
- Let's try to measure this so we can improve the algorithms.
- In the DataMap, under DIAGNOSTICS and if Algorithm Debug Logging is checked:
- Add some additional telemetry/metrics generated by PedDash
- Accuracy: % of times this algorithm has nailed the perfect and expected "FRAMES RX == 1".  This has to be an accuracy from 0 to 100 but with 3 decimal places.
- Please also add a metric showing the number of samples where FRAMES RX was 1 (the expected value) like algorithm successes
- Algorithm Samples:  The number of GET's commands issued by this current algorithm.  It doesn't matter how many frames were received after a GET command, 1 GET command with response is 1 sample.
- Algorithm Run Time: The time in seconds that this algorithm has been running (seconds and one decimal place).
- If the Algorithm is changed, those metrics get reset.  
- Also reset when you toogle ENABLE_ALGO_DEBUG, thank you for the catch!
- This is how I plan to objectively select the best one or select which one to improve.
- Don't forget to update the all their corresponding descriptions (short, long, hover, click, etc)
- Since I can't run the program here for you or give you json samples that you cannot see in real life, if algorithm debugging is checked, what would you add to the log so you can pinpoint what to change in the algorithms or how to design a new one? Think like this: I will bring you back that data and I will ask you to create a new improved and perfected algorithm to determine the ideal next update interval to achieve 1 frame as a response to the GET. Possibly to tune or design a new algorithm, we need visibility into timing error, queue dynamics, and jitter and store in a A compact per-sample debug record (stored in a ring buffer).  I already have an 'Export CSV' for events, so add a new 'Export Algo Debug CSV'.  Keep the size of the recommended structure the same size as the 'History Lines' configuration which defaults at 1000.
- In the main Racing panel, change the right and left 'LOG:' labels with 'GAME:' because is the game who is going to be seeing those values, not a LOGICAL entity. 'PHYS' is fine.
- Another issue: In the 'Signals & Events' tab, the activity history logs, show the last one first. However, the smaller activity log/ticker in the Racing tab, shows them with the last at the bottom which is wrong. I also want to see the last one at the top.
- Also review the source code for PedBridge and add any metric missing in the 'Data Map'.  PedBridge is the one that sends the metrics/telemetry to PedDash.  Make sure all the metrics are nicely grouped/categorized and sorted in 'Data Map'.
- Try to improve the long/short/names descriptions of the metrics if possible (when hover and click).  Don't remove the actual name of the metric in the short/long description.
...
<head>
  ...
  <title>PedDash Arcade - Production v19</title>
  <style>
    :root {
      --bg-color: #050508;
      --panel-bg: #0f0f16;
