...
# -----------------------------------------------------------------------------
$Strings = @{
    Banner           = "Fanatec Pedals Monitor & Bridge v2.4 started."
...
try {
    ...
    while ($Iterations -eq 0 -or $LoopCount -lt $Iterations) {
        ...
        $rawGas = 0; $rawClutch = 0
        $res = [Fanatec.Hardware]::GetPosition($JoystickID, [ref]$rawGas, [ref]$rawClutch)
