...
run_bench_loop(Options *opt)
{
    ...
    LARGE_INTEGER qpc_t0;
    LARGE_INTEGER qpc_t1;
    int have_qpc = QueryPerformanceFrequency(&qpc_freq) ? 1 : 0;
    LONGLONG block_active_qpc = 0;
    LONGLONG total_active_qpc = 0;

    HANDLE current_thread = GetCurrentThread();
    ULONGLONG cycles_start = 0u;
    ULONGLONG cycles_prev = 0u;
    int have_cycles = 0;
    ULONGLONG cpu_start_100ns = 0u;
    ULONGLONG cpu_prev_100ns = 0u;
    int have_cpu = 0;
    ...
    for (unsigned i = 0; i < opt->bench_iters; ++i) {
        ...
        if (have_qpc) {
            ...
            block_active_qpc += (qpc_t1.QuadPart - qpc_t0.QuadPart);
            total_active_qpc += (qpc_t1.QuadPart - qpc_t0.QuadPart);
        }
        ...
        if (block_iters == opt->bench_report_every || measured_done == opt->bench_iters) {
            ...
            if (have_cycles) {
                ...
                if (QueryThreadCycleTime(current_thread, &cycles_now)) {
                    block_cycles = cycles_now - cycles_prev;
            ...
            if (have_cpu) {
                ...
                if (get_thread_cpu_100ns(current_thread, &cpu_now_100ns)) {
                    block_cpu_100ns = cpu_now_100ns - cpu_prev_100ns;
                    total_cpu_100ns = cpu_now_100ns - cpu_start_100ns;
                    cpu_prev_100ns = cpu_now_100ns;
                    have_block_cpu = 1;
                    have_total_cpu = 1;
                } else {
                    have_cpu = 0;
            ...
            if (have_block_cycles)
                printf(" cycles/iter=%.2f", (double)block_cycles / (double)block_iters);
            else
            ...
            if (have_qpc)
                printf(" wall_us/iter=%.3f",
                       ((double)total_active_qpc * 1000000.0) /
                       ((double)qpc_freq.QuadPart * (double)measured_done));
            else
                printf(" wall_us/iter=N/A");

            if (have_total_cycles && have_total_cpu && total_cpu_100ns > 0u) {
                double total_eff_ghz =
                    ((double)total_cycles) / (((double)total_cpu_100ns * 1e-7) * 1e9);
